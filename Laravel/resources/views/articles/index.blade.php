...
<h1 class="navTxt" style="text-align: center">{{$campagne->nom}}</h1>
@endforeach
...
@foreach ($articles as $article)
  <form method="POST" action="{{route('Commandes.store')}}" class="col-md-3 col-sm-12">
    ...
    <div class="card flex justify-content-center col-12" >
        ...
        <img src="{{asset('img/articles/' . $article->image)}}"  class="card-img-top" alt="{{$article->nom}}"/>
        @elseif($article->image == NULL)
        <img src="{{asset('img/articles/notFound.png')}}" height="60%" width="60%" class="card-img-top" alt="Image non-disponible"/>
        @endif
        <div class="card-body">
          ...
          <p class="card-text">{{$article->prix}}$</p>
            ...
            <button type="submit" class="btn " style="background-color: #5D7687">
